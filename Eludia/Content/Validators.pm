...
sub vld_inn_10 {
	...
	$value =~ /^\d{10}$/ or $name1 ? die "#$name1#:$i18n->{inn_10_digits_count_fail}" : return $i18n -> {inn_10_digits_count_fail};
	...
	$checksum == 0 + substr ($value, -1, 1) or $name1 ? die "#$name1#:$i18n->{inn_checksum_fail}" : return $i18n -> {inn_checksum_fail};
...
sub vld_inn_12 {
	...
	$value =~ /^\d{12}$/ or $name1 ? die "#$name1#:$i18n->{inn_12_digits_count_fail}" : return $i18n -> {inn_12_digits_count_fail};
	...
	$checksum == 0 + substr ($value, -2, 1) or $name1 ? die "#$name1#:$i18n->{inn_checksum_fail}" : return $i18n -> {inn_checksum_fail};
	...
	$checksum == 0 + substr ($value, -1, 1) or $name1 ? die "#$name1#:$i18n->{inn_12_checksum_fail}" : return $i18n -> {inn_12_checksum_fail};
...
sub vld_inn {
	...
	if (length $value == 10) {
		...
		return $name1 ? die "#$name1#:$i18n->{inn_digits_count_alt}" : return $i18n -> {inn_digits_count_alt};
	}
...
sub vld_okpo {
	...
	$_REQUEST {$name} =~ /^\d{8}$/ or die "#$name#:$i18n->{okpo_digits_count_fail}";
	...
	if ($checksum_1 > 9) {
		$checksum_2 == 0 + substr ($_REQUEST {$name}, -1, 1) or die "#$name#:$i18n->{okpo_checksum_fail}";
	}
	else {
		$checksum_1 == 0 + substr ($_REQUEST {$name}, -1, 1) or die "#$name#:$i18n->{okpo_checksum_fail}";
	}
